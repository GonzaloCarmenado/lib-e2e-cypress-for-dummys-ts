...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  .section { padding: 16px 20px; border-bottom: 1px solid #2a3245; }
  .section:last-child { border-bottom: none; }
  h4 { margin: 0 0 12px; font-size: 12px; text-transform: uppercase;
       letter-spacing: 0.5px; color: #6c7a99; font-weight: 700; }
  label { display: flex; align-items: center; gap: 10px; font-size: 13px;
          color: #c9d1d9; cursor: pointer; margin-bottom: 10px; }
  label:last-child { margin-bottom: 0; }
  select {
    background: #0d1117; color: #fff; border: 1px solid #2a3245;
    border-radius: 6px; padding: 6px 10px; font-size: 13px; outline: none;
    cursor: pointer;
  }
  select:focus { border-color: #1976d2; }
  input[type="checkbox"] { width: 16px; height: 16px; cursor: pointer; accent-color: #1976d2; }
  .toggle-desc { font-size: 11px; color: #6c7a99; margin-left: 2px; }
...
export class ConfigurationElement extends HTMLElement {
  ...
  async exportAllData(): Promise<void> {
    ...
    a.href = url; a.download = 'e2e-cypress-export.json'; a.click();
  ...
  private render(): void {
    const langOptions = LANGS.map((l) =>
      `<option value="${l.value}" ${this.selectedLanguage === l.value ? 'selected' : ''}>${l.label}</option>`
    ).join('');
    ...
    (this.shadow.getElementById('lang-select') as HTMLSelectElement)
      .addEventListener('change', (e) => this.onLanguageChange((e.target as HTMLSelectElement).value));

    (this.shadow.getElementById('http-toggle') as HTMLInputElement)
      .addEventListener('change', (e) => this.onAdvancedHttpConfigChange((e.target as HTMLInputElement).checked));

    this.shadow.getElementById('btn-export')!
      .addEventListener('click', () => this.exportAllData());

    (this.shadow.getElementById('file-input') as HTMLInputElement)
